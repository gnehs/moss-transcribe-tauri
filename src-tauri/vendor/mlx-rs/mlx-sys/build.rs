...
fn build_and_link_mlx_c() {
    ...
    // Local development and reproducible CI may provide an already fetched MLX
    // source tree. Without this override, mlx-c keeps its normal FetchContent
    // behavior and resolves the pinned tag declared in CMakeLists.txt.
    if let Ok(source_dir) = env::var("MLX_SOURCE_DIR") {
        config.define("FETCHCONTENT_SOURCE_DIR_MLX", source_dir);
    }
    for (environment, dependency) in [
        ("METAL_CPP_SOURCE_DIR", "METAL_CPP"),
        ("FMT_SOURCE_DIR", "FMT"),
        ("JSON_SOURCE_DIR", "JSON"),
        ("GGUFLIB_SOURCE_DIR", "GGUFLIB"),
    ] {
        println!("cargo:rerun-if-env-changed={environment}");
        if let Ok(source_dir) = env::var(environment) {
            config.define(format!("FETCHCONTENT_SOURCE_DIR_{dependency}"), source_dir);
        }
    }
